...
class Backtester:
    ...
    def run(self, df, symbol='TEST/USDT'):
        ...
        print(f"\n🔍 Backtest {symbol} sur {len(df)} bougies...")
        ...
        for i in range(50, len(df) - 1):
            ...
            if position:
                ...
                if price <= sl or price >= tp or signal['action'] == 'SELL':
                    ...
                    trades.append({
                        'exit_price': price, 'entry_price': position['entry'],
                        'pnl': pnl, 'reason': 'SL' if price <= sl else ('TP' if price >= tp else 'SIGNAL')
                    })
                    position = None
        ...
        if wins: print(f" Meilleur trade: +{max(t['pnl'] for t in wins):.2f} USDT")
        if losses: print(f" Pire trade: {min(t['pnl'] for t in losses):.2f} USDT")
